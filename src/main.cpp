...
#if defined(NDEBUG)
# error "Globalboost cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfcf61c3e0e9a8822a93d90163c0ea58f7669fa633ea62c33e7c6bebc77467c64");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Globalboost: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Globalboost Signed Message:\n";
...
{
    // Globalboost: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	else if(nHeight < 4206200)
    {
        nSubsidy = 95 * COIN;
    }
	else if(nHeight < 8411000)
    {
        nSubsidy = 48 * COIN;
    }
	else if(nHeight < 12628800)
    {
        nSubsidy = 24 * COIN;
    }
    else
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Globalboost: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Globalboost: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 's';
        pchMessageStart[3] = 't';
        hashGenesisBlock = uint256("0xfcf61c3e0e9a8822a93d90163c0ea58f7669fa633ea62c33e7c6bebc77467c64");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "cnn.com New phase of MH370 search focuses on ocean floor";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 95 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1398668445;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 115256462;
        ...
        assert(block.hashMerkleRoot == uint256("0x0x8710e54cd11e0e1e9ebb9d765a9b838d5eccec27bf953e2b3c3917534bcc47bd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb1, 0xe2, 0xf2 }; // 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlobalboostMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlobalboostMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlobalboostMiner(CWallet *pwallet)
{
    printf("GlobalboostMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("globalboost-miner");
    ...
    try { loop {
        ...
        printf("Running GlobalboostMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GlobalboostMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlobalboostMiner, pwallet));
}
