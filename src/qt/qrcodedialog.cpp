...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BST")
    updateDisplayUnit();
}
...
{
    QString ret = QString("globalboost:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BST unit input in lnReqAmount, we generate the URI with BST as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BST, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
